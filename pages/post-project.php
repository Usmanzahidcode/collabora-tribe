...
$post_check_query = "SELECT last_posted FROM users WHERE id = ?";
$post_check_stmt = $conn->prepare($post_check_query);
$post_check_stmt->bind_param('i', $_SESSION['user_id']);
$post_check_stmt->execute();
$result = $post_check_stmt->get_result();
$row = $result->fetch_assoc();
$last_posted = $row['last_posted'];
$current_date = date('Y-m-d');


if ($last_posted == $current_date) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $current_date = date('Y-m-d');
        $query = "UPDATE users SET cp = cp + 5, last_posted = ? WHERE id = ?";
        $stmt = $conn->prepare($query);
        $stmt->bind_param('si', $current_date, $_SESSION['user_id']);
